...
## 柳沢PIC形式について
柳沢PIC形式は[X68000](https://ja.wikipedia.org/wiki/X68000)で使われた画像圧縮形式です。
1980年代の末に登場したもので、当時としては圧倒的な高圧縮率を誇りました。
当時、Oh!X 1990年2月号に解説記事が掲載されていたようです(ぼくはその記事を読んだことがあるかどうか、記憶が定かではありません)。

概要としては以下の記事が参考になると思います。
...
## 使用方法(エンコード)
以下のコマンドラインでPICファイルの作成(エンコード)を行います。
```
java -jar picloader2022.jar --encode 入力画像ファイル名 出力PICファイル名
```

仕様について
- 色はX68000用の32768色で行います(決め打ち)
- コメント文字列を入れるのは対応していません
- 画像解像度は触りません(大きなサイズの画像もそのままPICファイルを作成します)
- エンコードはCLIで呼び出す前提です(標準出力にメッセージを出力します)
...
PICでは色は初期値である黒色( $0000 )が設定されています。
データは初期値の黒色の長さから開始し、次の色、次の色の連鎖、次の色の長さの順にデータが繰り返します。
画面の左上は黒とは限らず、色の長さの符号化は 1 以上の数値を表現するため、フォーマット上は描画されることがない (-1, 0) の座標が存在することになっています。
この座標が黒固定みたいなイメージです。
画像左上(0,0)が黒色以外の場合、最初の初期値黒色の長さが1になり、画面左上(0,0)が黒色の場合は最初の長さが2以上になります。

![開始座標](./readme_fig/position.png)

### 長さ
長さは以下に例示されるように符号化されます(ワイル符号化。Wyle coding)。前半が[ハフマン符号](https://ja.wikipedia.org/wiki/%E3%83%8F%E3%83%95%E3%83%9E%E3%83%B3%E7%AC%A6%E5%8F%B7)みたいな感じで、後半が通常の固定桁の2進符号化。
...
上記の符号化により、同色の下方向への連鎖を記述します。符号に対応する位置は以下です。

![連鎖の符号の意味](./readme_fig/chain_pos.png)
...
![連鎖の説明:図１](./readme_fig/chain_fig1.png)

> すると図２のようになります。

![連鎖の説明:図２](./readme_fig/chain_fig2.png)
...
![連鎖の説明:図３](./readme_fig/chain_fig3.png)
