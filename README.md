...
(「本資料の転載、引用すべて自由です（連絡不要）」との記載に基づき、かなり多めに引用しています。
...
（キャッシュミス時）キャッシュされていない色を追加する動作では、テーブルは例えば以下のように変化します。

![キャッシュされていない色の追加1](./readme_fig/colorindex1.png)

![キャッシュされていない色の追加2](./readme_fig/colorindex2.png)

（キャッシュヒット時）キャッシュされている色を再利用する動作では、テーブルは例えば以下のように変化します。
...
色インデックス番号も符号化することで、データサイズをさらに小さくすることができるのかもと想像してしまいますが、データサイズ vs 展開速度の兼ね合いや、当時存在した画像ファイルでのデータの性質、等々が考慮された結果だと思います。

### 連鎖
![圧縮データの構造](./readme_fig/chain.png)

#### 連鎖の考え方
PICの圧縮は、ランレングス符号化（連長圧縮）の考え方を拡張したものです。
素のランレングス法は、1x1のドットの色が変わる位置までの長さを記録します。
PICでは色が変わるドット位置で、さらに連鎖形状を符号化します。
連鎖が符号化済の箇所は色が変化しないとして扱い、連長を長くし、変化点の数を劇的に減らすことができる(その場所の符号化をする必要がない)。
